...
const aggregate = require("./test-aggregate");
...
describe("CQRS event source", () => {
  ...
  broker1.createService({
    ...
    events: {
      "emitter.**": () => console.log("event..."),
    },
  });
  broker2.createService({
    name: "echo",
    actions: {
      reply: () => console.log("echo"),

      helloProxy(ctx) {
        return ctx
          .call("greeter.hello", ctx.params)
          .then(res => `${res} (proxied)`);
      },

      emitter(ctx) {
        this.broker.emit("emitter.hello.event", ctx.params);
      },
